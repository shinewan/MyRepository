            ...
            <div class="food-content">
                ...
                <div class="rating-wrapper">
                    <ul v-show="food.ratings && food.ratings.length">
                        <!-- v-show绑定一个函数needShow 根据评论的type 和 内容text有无 返回不同的状态 来控制(过滤)显示的内容 -->
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            // 将cartcontrol子组件派发的 add事件传递的dom 再次派发给 该组件的父组件goods   （儿子-->ba -->爷爷）
            // 然后 goods组件内 引用的food组件要通过@add=“addFood” 接收这个组件派发出去的dom，触发goods组件的函数（最终完成小球动画）
            addFood(target) {
                this.$emit('add', event.target);
            },
            // selectType 组件中通过派发事件 修改子组件的props
            selectRating(type) {
...
<style lang="stylus" rel="stylesheet/stylus">
    ...
    .food
        ...
        .image-header
            ...
            width: 100%
            height: 0
            // 当padding设为100%时，padding的高度是等于宽度的
            padding-top: 100%
            img
                position: absolute
                top: 0
